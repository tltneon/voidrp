...
timer.Create( "secondstimer", 60, 0, progressseconds)
timer.Start( "secondstimer" )
...
function daylight:think( )
	...
	if ( CAKE.ClockMonth > 12 ) then 
		local yearandshit = string.Explode( "-", CAKE.ClockYear )
		CAKE.ClockYear = tonumber( yearandshit[1]) + 1 .. "-ER" 
		CAKE.ClockMonth = 1
	end
