...
/*
function ccSetDeity( ply, cmd, args )

	local deity = args[ 1 ];
	
	if( ply:GetNWInt( "charactercreate" ) == 1 ) then
	
			CAKE.SetCharField(ply, "deity", deity );	
	end
	
	return;
end
concommand.Add( "rp_setdeity", ccSetDeity );*/

function ccSetHeight( ply, cmd, args )

	local height = tonumber( args[ 1 ] )
	
	if( ply:GetNWInt( "charactercreate" ) == 1 ) then
	
			CAKE.SetCharField(ply, "height", height );	
			CAKE.SetSize( ply, height )
	
	end
	
	return;
end
concommand.Add( "rp_setheight", ccSetHeight );
